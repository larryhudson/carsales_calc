# we need to declare a function that returns a list of cars with their attributes - price, kms, year
...
def get_search_url(search):
	make = search.car_make
	model = search.car_model
	state = search.state
	...
	return "http://www.carsales.com.au/cars/results?q=%28%28%28%28%28%28%28Make%3D%5B" + car_make + "%5D%26Model%3D%5B" + car_model + "%5D%29%26State%3D%5B" + state+ "%5D%29%26Service%3D%5BCarsales%5D%29" + "%26GenericGearType%3D%5B" + transmission + "%5D%29%26Price%3Drange%5B" + min_price + ".." + max_price + "%5D%29" + "%26Odometer%3Drange%5B" + min_kms + ".." + max_kms + "%5D%29%26%28BodyStyle%3D%5BHatch%5D%7CBodyStyle%3D%5BSedan%5D%29%29&sortby=Year&limit=24"
	
def get_car_list(search):
	search_url = get_search_url(search)
	soup = load_page(search_url)
	
	
	# make a list of car elements
	# page_listings = driver.find_elements_by_css_selector("div.listing-item")
	page_listings = soup.find_all('div',{'class':'listing-item'})
